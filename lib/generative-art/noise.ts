/**
 * Simplex Noise Implementation (compact 2D/3D)
 * Used for organic texture generation in agent art
 */
...
export function simplex2(xin: number, yin: number): number {
  ...
  if (t0 < 0) n0 = 0;
    ...
    t0 *= t0;
    n0 = t0 * t0 * dot2(GRAD3[PERM12[ii + PERM2[jj]]], x0, y0);
  }
  ...
  if (t1 < 0) n1 = 0;
    ...
    t1 *= t1;
    n1 = t1 * t1 * dot2(GRAD3[PERM12[ii + i1 + PERM2[jj + j1]]], x1, y1);
  }
  ...
  if (t2 < 0) n2 = 0;
    ...
    t2 *= t2;
    n2 = t2 * t2 * dot2(GRAD3[PERM12[ii + 1 + PERM2[jj + 1]]], x2, y2);
  }
...
export function simplex3(xin: number, yin: number, zin: number): number {
  ...
  if (t0 < 0) n0 = 0;
  else { t0 *= t0; n0 = t0 * t0 * dot3(GRAD3[PERM12[ii + PERM2[jj + PERM2[kk]]]], x0, y0, z0); }

  let t1 = 0.6 - x1 * x1 - y1 * y1 - z1 * z1;
  if (t1 < 0) n1 = 0;
  else { t1 *= t1; n1 = t1 * t1 * dot3(GRAD3[PERM12[ii + i1 + PERM2[jj + j1 + PERM2[kk + k1]]]], x1, y1, z1); }

  let t2 = 0.6 - x2 * x2 - y2 * y2 - z2 * z2;
  if (t2 < 0) n2 = 0;
  else { t2 *= t2; n2 = t2 * t2 * dot3(GRAD3[PERM12[ii + i2 + PERM2[jj + j2 + PERM2[kk + k2]]]], x2, y2, z2); }

  let t3 = 0.6 - x3 * x3 - y3 * y3 - z3 * z3;
  if (t3 < 0) n3 = 0;
  else { t3 *= t3; n3 = t3 * t3 * dot3(GRAD3[PERM12[ii + 1 + PERM2[jj + 1 + PERM2[kk + 1]]]], x3, y3, z3); }
