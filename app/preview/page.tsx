  ...
  return (
    <>
      ...
      <div className="mx-auto w-full max-w-[1320px]">
        <section className="mb-8 grid gap-6 rounded-[6px] border border-black/10 bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)] lg:grid-cols-[1.2fr_0.8fr] lg:p-8">
          <div>
            <p className="text-[10px] font-semibold tracking-[0.18em] text-black/45">PREVIEW GALLERY</p>
            <h1 className="mt-3 max-w-3xl text-[32px] font-semibold leading-[0.95] tracking-[-0.03em] text-[#1d1f21] sm:text-[40px] lg:text-[48px]">
              Compare the full cNFT visual set before you mint.
            </h1>
            <p className="mt-3 max-w-2xl text-[14px] leading-[1.55] text-black/65 sm:text-[15px]">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <Link href="/forge" className="inline-flex h-10 items-center justify-center rounded-[4px] bg-[#171819] px-5 text-[12px] font-semibold tracking-[0.08em] text-white transition-colors hover:bg-[#111111]">
                OPEN FORGE
              </Link>
              <Link href="/marketplace" className="inline-flex h-10 items-center justify-center rounded-[4px] border border-black/10 bg-white px-5 text-[12px] font-semibold tracking-[0.08em] text-black/70 transition-colors hover:bg-black/5">
                VIEW MARKETPLACE
              </Link>
            </div>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-[4px] border border-black/10 bg-[#f6f7f8] p-4">
              <p className="text-[10px] font-semibold tracking-[0.18em] text-black/45">DETERMINISTIC</p>
              <p className="mt-2 text-[18px] font-semibold tracking-[-0.02em] text-[#1d1f21]">One agent, one visual identity.</p>
              <p className="mt-1.5 text-[13px] leading-[1.5] text-black/60">Seeded from agent identity and mint metadata, so previews and minted artwork stay stable.</p>
            </div>
            <div className="rounded-[4px] border border-black/10 bg-[#f6f7f8] p-4">
              <p className="text-[10px] font-semibold tracking-[0.18em] text-black/45">MOTION</p>
              <p className="mt-2 text-[18px] font-semibold tracking-[-0.02em] text-[#1d1f21]">Animated SVG overlays.</p>
              <p className="mt-1.5 text-[13px] leading-[1.5] text-black/60">Orbit rings, drifting glows, and subtle float motion keep the art alive without changing mint flow.</p>
            </div>
            <div className="rounded-[4px] border border-black/10 bg-[#f6f7f8] p-4 sm:col-span-2">
              <p className="text-[10px] font-semibold tracking-[0.18em] text-black/45">HOW TO READ</p>
              <p className="mt-2 text-[13px] leading-[1.55] text-black/65">
          ...
          {previewAgents.map((agent) => (
            <article key={agent.id} className="overflow-hidden rounded-[6px] border border-black/10 bg-white shadow-[0_1px_4px_rgba(0,0,0,0.04)] transition-shadow hover:shadow-md">
              <div className="relative aspect-[4/5] overflow-hidden bg-[#0b0f16]">
                ...
                <div className="absolute left-2.5 top-2.5 rounded-[2px] bg-[#171819] px-2.5 py-1 text-[9px] font-semibold tracking-[0.12em] text-white/80">
                  {agent.riskLevel.toUpperCase()}
                </div>
                <div className="absolute bottom-2.5 left-2.5 right-2.5 flex items-end justify-between gap-2">
                  <div>
                    <p className="text-[9px] font-semibold tracking-[0.15em] text-white/40">SEED {agent.seed}</p>
                    <h2 className="mt-0.5 text-[18px] font-semibold tracking-[-0.02em] text-white">{agent.name}</h2>
                  </div>
                  <span className="shrink-0 rounded-[2px] bg-[#e7f3f2] px-2 py-0.5 text-[9px] font-semibold tracking-[0.12em] text-[#171819]">
              ...
              <div className="space-y-3 p-3">
                <p className="min-h-[56px] text-[12px] leading-[1.55] text-black/70">
                  {agent.tagline}
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {agent.tools.slice(0, 3).map((tool) => (
                    <span key={tool} className="rounded-[2px] bg-black/[0.04] px-2 py-0.5 text-[9px] font-medium tracking-[0.08em] text-black/60">
                      {tool}
                    </span>
                  ))}
                </div>
