...
import { generatePalette } from "@/lib/generative-art/palette";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { agentId, name, personality, riskLevel, toolCount, seed } = body;
    ...
    const fileName = `vessel-${agentId.slice(0, 8)}.png`;

    const result = await uploadCnftImage(buffer, fileName, "/cnft/agents");
    ...
    console.error("cNFT generation + upload error:", error);
    return NextResponse.json(
      { error: "Failed to generate and upload cNFT" },
      { status: 500 }
    );
  }
}
...
interface DrawConfig {
  palette: { primary: string; secondary: string; accent: string; background: string; highlight: string };
  size: number;
  seed: number;
  name: string;
  personality: string;
...
function drawEnhancedCnft(ctx: CanvasRenderingContext2D, config: DrawConfig): void {
  const { palette, size, rng, cx, cy } = config;

  drawBackground(ctx, config);
  drawAtmosphere(ctx, config);
  drawBody(ctx, config);
  drawSuit(ctx, config);
  drawNeck(ctx, config);
  drawHead(ctx, config);
  ...
  drawImplants(ctx, config);
  drawEffects(ctx, config);
...
function drawBackground(ctx: CanvasRenderingContext2D, { palette, size, rng }: DrawFnConfig): void {
    ...
    case "crystal":
      for (let i = 0; i < 12; i++) {
        ...
        for (let j = 0; j < 6; j++) {
          ...
          ctx[j === 0 ? "moveTo" : "lineTo"](Math.cos(angle) * s, Math.sin(angle) * s);
        }
        ctx.closePath();
        ctx.strokeStyle = palette.primary + "0a";
        ctx.lineWidth = 1;
...
function drawAtmosphere(ctx: CanvasRenderingContext2D, { palette, size, rng }: DrawFnConfig): void {
  for (let i = 0; i < 40; i++) {
    const x = rng.nextFloat(0, 1) * size;
    const y = rng.nextFloat(0, 1) * size;
...
function drawBody(ctx: CanvasRenderingContext2D, { palette, size, rng, cx = 0, cy = 0 }: DrawFnConfig): void {
...
function drawSuit(ctx: CanvasRenderingContext2D, { palette, size, rng, cx = 0, cy = 0 }: DrawFnConfig): void {
...
function drawNeck(ctx: CanvasRenderingContext2D, { palette, size, rng, cx = 0, cy = 0 }: DrawFnConfig): void {
...
function drawHead(ctx: CanvasRenderingContext2D, { palette, size, rng, cx = 0, cy = 0 }: DrawFnConfig): void {
...
function drawHair(ctx: CanvasRenderingContext2D, { palette, size, rng, cx = 0, cy = 0 }: DrawFnConfig): void {
...
function drawEyes(ctx: CanvasRenderingContext2D, { palette, size, rng, cx = 0, cy = 0 }: DrawFnConfig): void {
...
function drawNose(ctx: CanvasRenderingContext2D, { palette, size, cx = 0, cy = 0 }: DrawFnConfig): void {
...
function drawMouth(ctx: CanvasRenderingContext2D, { palette, size, rng, cx = 0, cy = 0 }: DrawFnConfig): void {
...
function drawImplants(ctx: CanvasRenderingContext2D, { palette, size, rng, cx = 0, cy = 0, toolCount = 3 }: DrawFnConfig): void {
...
function drawEffects(ctx: CanvasRenderingContext2D, { palette, size, rng, cx = 0, cy = 0 }: DrawFnConfig): void {
...
function drawFrame(ctx: CanvasRenderingContext2D, { palette, size, rng, riskLevel = "Balanced", name = "" }: DrawFnConfig): void {
  const m = size * 0.03;
